...
static bool has_o_flag(int argc, char **argv) {
  for (int i = 1; i < argc; ++i) {
    const char *a = argv[i];
    if (!a) continue;
    if (streq(a, "-o")) return true;
    if (!strncmp(a, "-o", 2) && a[2]) return true;
  }
  return false;
}
...
static void inject_default_out(int *pargc, char ***pargv, const char *out) {
  ...
  if (!(is_cmd(argc, argv, "run") || is_cmd(argc, argv, "build"))) return;
  if (has_o_flag(argc, argv)) return;
  ...
  for (int i = 2; i < argc; ++i) newv[k++] = argv[i];
  newv[k] = NULL;
